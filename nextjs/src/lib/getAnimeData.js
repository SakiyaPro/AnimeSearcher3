...
// 声優名で取得
export async function getCastAnimeData(castName, { offset = 0 }) {
    const res = await (await axios.get(
        encodeURI(`${process.env.NEXT_PUBLIC_DJANGO_URL}api/animedata/?format=json&limit=30&offset=${offset}&casts=${castName}&watchersCount_max=true`))
    ).data.results
    return res
}

// スタッフ名で取得
export async function getStaffAnimeData(castName, { offset = 0 }) {
    const res = await (await axios.get(
        encodeURI(`${process.env.NEXT_PUBLIC_DJANGO_URL}api/animedata/?format=json&limit=30&offset=${offset}&staffs=${castName}&watchersCount_max=true`))
    ).data.results
    return res
}
